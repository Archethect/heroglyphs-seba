...
contract FallbackTest is BaseTest {
    function test_WhenValueIsZero() external {
        ...
        vm.expectEmit();
        emit IBoostPool.EtherReceived(address(this), 0);
        (bool success, ) = address(boostPool).call{ value: 0 }("");
        assertTrue(success);
    }
    ...
    function test_WhenValueIsNotZero(uint256 amount) external {
        ...
        vm.expectEmit();
        emit IBoostPool.EtherReceived(address(this), amount);
        (bool success, ) = address(boostPool).call{ value: amount }("");
        assertTrue(success);
    }
}
